...
/*** TO BE DONE START ***/
	...
	if (impl->pathname == NULL)
		p = vt_lookup(sh_get_var_table(sh), HOME);
	else
		p = impl->pathname;

	if (chdir(p) == -1)
		fail_errno("Chdir error");

	cwd = getcwd(NULL, 0);
	if (cwd == NULL)
		fail_errno("Getcwd error");
	vt_set_value(sh_get_var_table(sh), PWD, cwd);
	free(cwd);
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	char *parsing = my_strdup(path), *headParsing = parsing, *pathname, *duepunti;
	ptrdiff_t len;
	do {
		duepunti = strchr(parsing, DIR_SEPARATOR);
		if (duepunti != NULL)
			len = duepunti - parsing;
		else
			len = strlen(parsing);

		pathname = my_malloc(len + strlen(name) + 2);
		strncpy(pathname, parsing, len);
		pathname[len] = 0;
		strcat(pathname, "/");
...
/*** TO BE DONE END ***/
	if (pid == 0) {
		 ...
		 * la syscall fallisce. */
/*** TO BE DONE START ***/
	if (execve(executable, impl->args, sh_get_environment(sh)) == -1)
		fail_errno("Execve error");
/*** TO BE DONE END ***/
	}
	cleanup_and_return:
	free(executable);
...
/*** TO BE DONE START ***/
	...
	for (i=0; i < 2; ++i) {
		if ((fcntlGet = fcntl(pipes[i], F_GETFD)) == -1)
			fail_errno("Fcntl get error");
		if (fcntl(pipes[i], F_SETFD, fcntlGet | FD_CLOEXEC) == -1)
			fail_errno("Fcntl set error");
	}
