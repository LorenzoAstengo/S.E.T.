...
/*** TO BE DONE 3.0 START ***/
		...
		file_size = 0;
		while ((header_sent = sendfile(client_fd, fd, &file_size, header_size)) < header_size) {
			if (header_sent == -1)
				fail_errno("incApache: could not send HTTP fd");
			file_size += header_sent;
			header_size -= header_sent;
		}
		close(fd);
/*** TO BE DONE 3.0 END ***/
